...
Read in data.
```{r}
raw_df <- read.csv("https://raw.githubusercontent.com/willoutcault/stock_preprocessing/master/stock_data.csv", T, ",")
...
ts_filter <- function(clean_df, stock, dates, t){
    df <- clean_df %>% 
      filter(as.Date(time, format="%m/%d/%y") %in% dates[seq(1,t,1)]) %>% 
...
for (t in timestep){
  ...
  dates <- unique(as.Date(raw_df$time, format="%m/%d/%y"))
