...
for (t in timestep){
  ...
  for (stock in stock_samples){
    ...
    #if (t==1){
      try(training1 <- ROSE(direction ~ ., training1, p=0.33, N=round(.5*nrow(testing), 0))$data)
      try(training2 <- ROSE(direction ~ ., training2, p=0.33, N=round(.5*nrow(testing), 0))$data)
